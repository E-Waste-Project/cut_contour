...
class CompetitionOperations():
    ...
    def key_hole_cb(self, msg):
        ...
        key_hole_pose = msg
        key_hole_pose.position.x += self.x_comp
        key_hole_pose.position.y += self.y_comp
        ...
        self.key_ms.move_to_touch(
            pose_array, vel_scale=0.01, acc_scale=0.01, axis='z', force_thresh=2)
        print("Moved To Key!!!")
        ...
        pose_array = self.key_ms.shift_pose_by(
            ...
            trans=[0.0, 0.08, 0], new_frame="gripper_key_fixed_frame")

        self.key_ms.move_to_touch(
            pose_array, axis='y', force_thresh=2, ref_frame=pose_array.header.frame_id)
        
        gripper_key_pose = self.tf_services.lookup_transform(
            "gripper_key_fixed_frame", "gripper_key")
        ...
        # move to touch in +x wrt gripper_hole_center_frame
        pose_array = self.key_ms.shift_pose_by(
            tf_services=self.tf_services, ref_frame="base_link", moving_frame="gripper_key",
            trans=[0.05, 0.0, 0], new_frame="gripper_center_frame")
        ...
        self.key_ms.move_to_touch(
            pose_array, axis='z', force_thresh=2, ref_frame=pose_array.header.frame_id)
