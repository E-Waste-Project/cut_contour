...
class CompetitionOperations():
    def __init__(self):
        ...
        rospy.Subscriber("/ethernet_empty_spiral_topic", PoseArray, self.ethernet_empty_spiral_cb)
    ...
    def battery_hole_spiral_cb(self, msg):
        ...
        if self.hole_pose is not None:
            ...
            shift_z = self.first_detected_hole_pose.position.z - self.hole_pose.position.z
            for pose in pose_array.poses:
                pose.position.x -= shift_x
                pose.position.y -= shift_y
                pose.position.z -= shift_z
        else:
        ...
        if self.hole_pose is None:
            self.hole_pose = self.tf_services.lookup_transform("capture_frame", "gripper_battery")
    ...
    def ethernet_cb(self, msg):
        ...
        self.ethernet_ms.move_group.set_pose_reference_frame(pose_array.header.frame_id)
        ...
        self.ethernet_ms.move_to_touch(ethernet_array, axis='xy', force_thresh=2)
        ...
        result = self.ethernet_ms.move_straight(ethernet_array, vel_scale=0.01, acc_scale=0.01)
        print("Moved Upwards!!!")
        if not result:
            rospy.sleep(5)
        ...
        # move upwards
        ethernet_pose = self.tf_services.lookup_transform("base_link", "gripper_ethernet")
    ...
    def ethernet_empty_spiral_cb(self, msg):
        ...
        pose_array = self.tf_services.transform_poses(
            "base_link", "capture_frame", pose_array)
        gripper_ethernet_pose = self.tf_services.lookup_transform("base_link", "gripper_ethernet")
        for pose in pose_array.poses:
        ...
        ethernet_array = self.tf_services.transform_poses("base_link", ethernet_array.header.frame_id, ethernet_array)
        self.ethernet_ms.move_group.set_pose_reference_frame(ethernet_array.header.frame_id)
        ...
        ethernet_array.poses.append(ethernet_pose)
        ethernet_array.poses[0].position.z -= 0.05
        self.ethernet_ms.move_to_touch(ethernet_array, axis='xy', force_thresh=5)
        ...
        # Move spirally until sudden change in z force.
        self.ethernet_ms.move_to_touch(pose_array, axis='xy', force_thresh=4)
        ...
        # ethernet_array.poses[0].position.z -= 0.005
        # result = self.ethernet_ms.move_straight(ethernet_array, vel_scale=0.01, acc_scale=0.01)
    ...
    def key_cb(self, msg):
        ...
        pose_array.poses[0].position.z += 0.03
        self.key_ms.move_group.set_pose_reference_frame(pose_array.header.frame_id)
        ...
        pose_array = PoseArray()
        pose_array = deepcopy(key_array)
        result = self.key_ms.move_straight(pose_array, vel_scale=0.01, acc_scale=0.01)
        print("Moved To Key!!!")
        if not result:
            rospy.sleep(5)
